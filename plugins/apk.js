...
const handler = async (m, {conn, usedPrefix: prefix, command, text}) => {
 if (!text) throw `*معشوق الجماهير هذا الأمر خاص بتحميل التطبيقات المجانية والمدفوعة منها نكتب هكذا على سبيل المثال* \n*.apk facebbok lite*`;
  try {    
    ...
    let response = `📲 تحميل التطبيقات 📲\n\n📌 *اسم التطبيق:* ${data5.name}\n📦 *الباكيدج:* ${data5.package}\n🕒 *تحذيث رقم:* ${data5.lastup}\n📥 *حجم التطبيق:* ${data5.size}\n\n*ما الذي يجعلك لا تتابع  صاحب البوت يا عزيزي 🙂 عمران يحب من يستعمل بوتاته لذا تابعه في حساباته* *https://instagram.com/hlimi_imran*  *💖صفحتي على الفيسبوك https://www.facebook.com/profile.php?id=61557052854167&mibextid=ZbWKwL  تابع صاحب البوت في صفحته على الفيسبوك لتشجيعه*`
